"""Story ↔ incident → release / service / postmortem (Sprint 8)."""
...
def test_ops_trace_s1842(tmp_path: Path) -> None:
    conn = _conn(tmp_path)
    bundle = json.loads(
    ...
    out = story_ops_trace_from_graph(conn, "S-1842")
    conn.close()
    assert out["ok"] is True
    assert out.get("linked") is True
    assert len(out.get("incidents") or []) >= 1
    assert len(out.get("releases_from_incidents") or []) >= 1
    assert len(out.get("services_impacted") or []) >= 1
    assert len(out.get("postmortems") or []) >= 1
