"""Story ↔ security findings, exceptions, controls (Sprint 6)."""
...
def test_security_trace_s1842(tmp_path: Path) -> None:
    conn = _conn(tmp_path)
    bundle = json.loads(
    ...
    out = story_security_trace_from_graph(conn, "S-1842")
    conn.close()
    assert out["ok"] is True
    assert out.get("linked") is True
    assert len(out.get("security_findings") or []) >= 2
    assert len(out.get("compliance_exceptions") or []) >= 1
    assert len(out.get("controls_for_releases") or []) >= 1
    assert len(out.get("releases_from_delivery") or []) >= 1
