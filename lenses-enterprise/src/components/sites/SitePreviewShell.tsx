...
import { ObjectMetaBar, PageHeader, StatePanel } from '../page'
import { useWorkspace } from '../../context/WorkspaceContext'
import { KNOWLEDGE_PUBLISH_COPILOT, STUDIO_VIEWER, STUDIO_VOCAB as V } from '../../nav/studioVisibleCopy'

export type SitePreviewShellProps = {
  siteName: string
  /** Path after the site folder (no leading slash). */
  subpath?: string
  /** When true, validate site name against workspace scan (Publish browse). */
  validateWorkspace?: boolean
}

function buildLocalSiteSrc(siteName: string, subpath: string): string {
  const decoded = decodeURIComponent(siteName).trim()
  if (!decoded) return ''
  const tail = (subpath || '').replace(/^\/+/, '')
  const base = `/local-site/${encodeURIComponent(decoded)}`
  return tail ? `${base}/${tail}` : `${base}/`
}

/** Unified static site preview chrome for Publish browse and legacy /view/local-site redirects. */
export function SitePreviewShell({
  siteName,
  subpath = '',
  validateWorkspace = true,
}: SitePreviewShellProps) {
  const decoded = decodeURIComponent(siteName)
  const src = buildLocalSiteSrc(siteName, subpath)
  ...
  if (validateWorkspace && loading && !state) {
    return (
      <>
        ...
        <StatePanel
          ...
          description="Checking published site names from the latest scan before opening the preview."
        />
      </>
    )
  }

  if (validateWorkspace && error) {
    return (
      <>
        <PageHeader title={V.siteBrowse} subtitle={STUDIO_VIEWER.siteBrowsePageSubtitle} />
        <StatePanel
    ...
    return (
      <>
        ...
        <StatePanel
          ...
          description="Pick a published site from the Websites list so the path includes its folder name."
          actions={
  ...
  return (
    <>
      ...
      <EmbeddedPreviewFrame
        ...
        disclosureKind="local-site-static"
      />
    </>
  )
}
