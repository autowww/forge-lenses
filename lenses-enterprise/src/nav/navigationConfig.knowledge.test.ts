...
describe('getSideNavEntries (Sprint UX6 — Knowledge & Publish grouping)', () => {
  it('groups Knowledge sidebar entries by learn / evidence / govern / build', () => {
    ...
    expect(groups.filter((g) => g === 'knowledge_govern').length).toBeGreaterThanOrEqual(2)
    expect(groups.filter((g) => g === 'knowledge_govern').length).toBeLessThanOrEqual(3)
    expect(groups.filter((g) => g === 'knowledge_labs').length).toBe(1)
  ...
  it('groups Publish sidebar entries by sites vs stories', () => {
    ...
    expect(groups.filter((g) => g === 'publish_stories').length).toBeGreaterThanOrEqual(2)
  })
})
