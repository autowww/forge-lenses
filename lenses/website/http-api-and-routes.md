...
**OIDC** — Optional env: **`LENSES_OIDC_ISSUER`**, **`LENSES_OIDC_CLIENT_ID`**, optional **`LENSES_OIDC_CLIENT_SECRET`**, **`LENSES_OIDC_REDIRECT_PATH`** (default **`/api/auth/oidc/callback`**), **`LENSES_OIDC_SCOPES`**. Loopback or **`LENSES_ALLOW_ACTIONS=1`** for the login/callback dance.
