...
| `GET` | `/api/nested-roadmap-config?repo=&roadmap_p=` | Structured nested-roadmap config for Studio **`NestedRoadmapHorizon`** React host (same aggregation as **`/nested-roadmap-view.html`**). |
...
| `GET` | `/api/today-charge?wbs_p=<rel>&repo=<repo_hint>&roadmap_p=<rel optional>` | JSON: **Today (Charge)** operational view — **`profile`** (`spark` \| `epic`), **`spark_rows`** or **`epic_rows`** (full list with **`flags`**, **`breadcrumb`**, **`plan_href`**), **`sections`** ( **`active`**, **`blocked`**, **`banked`**, **`recently_resolved`**, **`pending_versona`** ), **`charge`** (frontmatter hat/date, **`view_href`**), **`phase_prefixes`**, **`notes`**. Joins **`forge/charge.md`** (Active Sparks or **Active Epics** + Blockers + Banking tables) with WBS and Versona session index. **400** if `wbs_p` missing; **404** if WBS path not allowed. |
| `GET` | `/api/epic-spec-board?wbs_p=<rel>&repo=<repo_hint>&roadmap_p=<rel optional>` | JSON: **Spec Flow board** — derived Kanban (**`columns`**, **`cards`** with **`column`**, **`change_slug`**, **`overlays`**). **`profile`** `epic` when Charge uses Active Epics or **`openspec/config.yaml`** schema **`forge-sdlc`**; otherwise **`spark`** with empty **`cards`**. **400** / **404** same as today-charge. |
| `GET` | `/api/epic-hub?id=<M#E#>&wbs_p=<rel>&repo=<repo_hint>&roadmap_p=<rel optional>` | JSON: Epic hub — **`proposal_excerpt`**, **`spec_excerpt`**, **`validate_ok`**, **`size_gate`** (includes **`wiki_stale`**), **`dual_wiki`** (`stale`, `sides`, `reasons`, `refresh_allowed`), **`charge_row`**, derived **`column`**. **404** if Epic unknown. |
| `POST` | `/api/epic-spec-board/transition` | Body: **`epic_id`**, **`to_column`**, **`wbs_p`**, optional **`repo`**, **`change_slug`**, **`actor`**. Writes Charge / scaffolds OpenSpec per Spec Flow drag table. **403** non-loopback (same privilege as sticker-board save). **409** on illegal moves (e.g. Specify→Ready without validate green or **`wiki_stale`**). |
| `POST` | `/api/epic-spec-board/dual-wiki-refresh` | Body: **`epic_id`**, **`wbs_p`**, optional **`repo`**, **`change_slug`**. Loopback rebuild of local handbook HTML via **`scripts/refresh-dual-wiki.sh`** — **no Firebase**. **403** non-loopback. Does not call transition or ForgeRun. |
...
| `GET` | `/api/timeline-context?repo=&wbs_p=&roadmap_p=` | JSON for **Timeline** (Lenses Studio): **`repo_hints`**, **`wbs_options`**, **`roadmap_options`**, **`selected`**, **`gantt_bars`**, structured **`metrics`** (`horizon_counts`, `epic_bars`), **`date_rows`**, optional legacy **`gantt_html`**, **`metrics_html`**, **`editor_html`** (inspect-only), **`roadmap_source_href`**, **`workspace_projects`**, **`current_project`**. When the orchestration graph is enabled, may include **`orchestration_portfolio`**. |
...
`GET /api/timeline-context` includes structured **`date_rows`** (Initial/Target epic dates) and **`metrics`** for the Studio React `RoadmapDateEditor` and `TimelineMetrics` components. Legacy **`editor_html`** / **`metrics_html`** remain for inspect-mode parity only.
