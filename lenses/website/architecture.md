...
2. **Each GET** (except static `/docs`, `/__ks/…`, and project stats JSON) — The handler runs `scan_workspace` then optionally **`enrich_workspace_with_standards`** so each child includes **`standards_compliance`** (score, tier, checks, suggestions). Set **`LENSES_STANDARDS_ENRICH=0`** to skip enrichment (faster scans; overview compliance charts empty). Results are **cached briefly** (default TTL ~20 seconds; Studio Electron defaults **`LENSES_SCAN_CACHE_SEC=120`**; set env to adjust, or `0` to disable). **`?refresh=1`** on the URL bypasses the cache and forces a new scan. HTML uses `git_extended=True`; **`GET /api/workspace-state`** uses `git_extended` only when `?git_extended=1` is passed (compliance is attached when enrichment runs).
3. **Overview telemetry jobs** — When **`LENSES_OVERVIEW_ASYNC=1`** (default), **`GET /api/chart-data/overview`** serves a fresh cached snapshot when within TTL (**`LENSES_OVERVIEW_CACHE_SEC`**, default 300s) or starts an async job (max **3** parallel workers) and may return **202** with **`job_id`** + optional **`stale_snapshot`**. Poll **`GET /api/jobs/overview/{id}`** for **`progress`** (`phase`, `done`/`total`, `detail`) and the final payload. **`POST /api/jobs/overview`** `{ horizon, force }` starts or attaches to the same queue. Last-good payloads persist under **`.lenses-local/overview-cache.json`**. Set **`LENSES_OVERVIEW_ASYNC=0`** to restore synchronous overview builds on every request.
4. **JSON API** — `GET /api/workspace-state` returns the scan dict; `GET /api/roadmap-outline?p=…` returns section metadata for one roadmap file; `GET /api/project/<name>/stats` returns repo statistics; **`GET /api/project/<name>/chart-data`** returns JSON for client-side charts (see charts-api page); `POST /api/project/<name>/git` runs allowlisted git commands (loopback-gated by default); `POST /api/toolset/run` runs an allowlisted workspace-root **`*.sh`** (same loopback gate as git by default); `GET`/`POST /api/sticker-board?board_id=…` reads/writes one board (local or shared split under **`sticker-boards/<id>.*`**); `GET`/`POST /api/sticker-board-registry` lists and mutates the board registry (POST loopback-gated like git by default).
