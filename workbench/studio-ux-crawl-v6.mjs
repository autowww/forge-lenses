#!/usr/bin/env node
/**
 * Forge Lenses Studio UX crawl v6 — post FLS5 UX5 PDCA remediation.
 * Usage: node workbench/studio-ux-crawl-v6.mjs [baseUrl]
 * Output: workbench/studio-ux-crawl-v6/report.json
 */
...
const OUT_DIR = path.join(__dirname, 'studio-ux-crawl-v6')
...
function analyze(route, data) {
  ...
  if (route.startsWith('/view/local-site/') && !String(data.path).includes('/websites/browse/')) {
    issues.push('local_site_redirect_missing')
  }
...
const summary = {
  ...
  redirectRoutes: results.filter((r) => r.analysis?.issues?.includes('local_site_redirect_missing')).map((r) => r.route),
  results,
}
