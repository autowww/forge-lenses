#!/usr/bin/env bash
# Run an FL Studio UX PDCA phase via Cursor CLI (`agent`).
set -euo pipefail
...
if [[ -z "$PHASE" ]]; then
  echo "Usage: $0 [REPO_ROOT] <S00|…|S12>" >&2
  exit 1
fi

PROMPT_GLOB="docs/prompts/fl-studio-ux-pdca/${PHASE}*.md"
shopt -s nullglob
matches=( ${PROMPT_GLOB} )
shopt -u nullglob
...
ROOT="$(pwd)"
MODEL="${FL_STUDIO_UX_PDCA_MODEL:-composer-2.5}"
...
Follow Plan → Do → Check → Act. Run the phase gate when the prompt says Check:
  scripts/fl-studio-ux-pdca/check-phase-gate.sh ${PHASE}
...
if [[ -n "${FL_STUDIO_UX_CURSOR_AGENT_EXTRA:-}" ]]; then
  read -r -a USER_FLAGS <<< "${FL_STUDIO_UX_CURSOR_AGENT_EXTRA}"
