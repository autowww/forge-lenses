#!/usr/bin/env bash
# Run an FL Studio UX3 PDCA phase via Cursor CLI (`agent`).
set -euo pipefail
...
if [[ -z "$PHASE" ]]; then
  echo "Usage: $0 [REPO_ROOT] <U00|…|U08>" >&2
  exit 1
fi

PROMPT_GLOB="docs/prompts/fl-studio-ux3-pdca/${PHASE}*.md"
shopt -s nullglob
matches=( ${PROMPT_GLOB} )
shopt -u nullglob
...
ROOT="$(pwd)"
MODEL="${FL_STUDIO_UX3_PDCA_MODEL:-composer-2.5}"
...
Follow Plan → Do → Check → Act. Run the phase gate when the prompt says Check:
  scripts/fl-studio-ux3-pdca/check-phase-gate.sh ${PHASE}
...
if [[ -n "${FL_STUDIO_UX3_CURSOR_AGENT_EXTRA:-}" ]]; then
  read -r -a USER_FLAGS <<< "${FL_STUDIO_UX3_CURSOR_AGENT_EXTRA}"
